					...
					for (int i=255;i >= 0;i--)
{
	...
	public void onNextClick(View p1)
	{
		...
		if(!lock){
			try{
		if(tr.needAppend()){tr.changeText(dataArr.get(readPos));readPos++;
		}if(tr.needclean){Klog.v(readingText.getText());
		logTextView.setText(Klog.log.trim());
		}
		tr.type();}catch(IndexOutOfBoundsException e){
...
class Typer{
	...
	public boolean needAppend(){return msgque.isEmpty();}
...
isTyping=true;
					...
					boolean inMark=false;
							while(!msgque.isEmpty()&&!needexit){
								...
								deley(delay);
								try{
								if(inMark&&!marks.contains(msgque.peek())){needexit=true;
								if(curText.contains(".")||
									curText.contains("?")||
									curText.contains("!")||
									//curText.contains(":")||
									...
									curText.contains("。")||
										...
										curText.contains("？")||
										curText.contains("！")
								){needclean=true;}}}catch(NullPointerException e){needexit=true;needclean=true;}
								l=s;
					}
					if(msgque.isEmpty()){
					needclean=true;}
